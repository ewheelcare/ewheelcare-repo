...
$sql = "update sales_trans set pay_type='".$pay_type."',ref_no='".$ref_no."',pending=pending-".$paid_amount.",paid_amount=".$paid_amount." WHERE trans_id='".$trans_id."'";
echo $sql;
...
$sql = "INSERT INTO payments (
            ...
            '$payment_id', STR_TO_DATE('$payment_date', '%d-%m-%Y'), '$pay_type', 
            '$customer', '$customer_name', '$pay_type', '$account', $paid_amount,
            '$ref_no', 'A', '" . $_COOKIE["shop"] . "', NOW(),'".$user_id."'
        )";
$result = $conn->query($sql);

$update_master = "update account set balance=balance+".$paid_amount ." where account_id='".$account."'"	;	
$conn->query($update_master);	
$insert_pay_track = "insert into pay_track(payment_id,amount_settled,trans_id,mode) values('".$payment_id."','".$paid_amount."','".$trans_id."','sales')";
