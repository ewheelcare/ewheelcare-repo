...
$result = $conn->query($sql);
///echo $sql;
$sql = "SELECT sum(total) GRAND_TOTAL, sum(tax_amount) TAX_AMOUNT,sum(tax_amount_sgst) TAX_AMOUNT_SGST, sum(tax_amount_igst) TAX_AMOUNT_IGST,sum(price) PRICE  FROM receipt_trans_det where trans_id='".$trans_id."' and active_status!='Z'";
$result = $conn->query($sql);
if($row = $result->fetch_assoc()) {
	$grand_total=$row["GRAND_TOTAL"] ;
...
//echo $sql;*/
echo $subtrans_id."~".$grand_total."~".$tax_amount."~".$tax_amount_sgst."~".$tax_amount_igst."~".$price;
$conn->close();
?>