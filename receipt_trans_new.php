...
// echo $trans_date."====". $customer."====". $CUSTOMER_NAME;
						...
						<div class="col-md-12">
							...
							<table class="table">
								<tr>
									<td>Date<input type="text" id="datepicker" class="form-control" onchange="saveHeader()"><br>
									...
									<td>Trans No <input class="form-control" readonly name="trans_id" id="trans_id"
										...
										Search vendor<br><select class="form-control" name="customer_search"
											id="customer_search" multiple="multiple" style="max-width:300px!important">
											<option value=""></option>
...
<a href="module.php?param=vehicle" class="btn btn-danger" target="_blank">Add Vehicle, If not exists</a>-->
									<input class="form-control" name="customer" id="customer"
										value="<?php echo $customer; ?>" type="hidden">
									</td>
								</tr>
							</table>
						</div>
							...
							<table class="table">
								<tr>
									<td>Company Name<input class="form-control" name="company_name" id="company_name"
										...
										<br>Mobile <input class="form-control" name="customer_mobile"
											id="customer_mobile" value="<?php echo $CUSTOMER_MOBILE; ?>">
									</td>
									...
									<td style="display:<?php echo $visibility; ?>">GST <input class="form-control"
											...
											list="gst_list" onfocus="get_gst()" autocomplete="off" onblur="set_gst();saveHeader();">
										<datalist id="gst_list">
										</datalist><br>Address <input class="form-control" name="customer_address"
											id="customer_address" value="<?php echo $CUSTOMER_ADDRESS; ?>"
											list="address_list" onfocus="get_address()" autocomplete="off" onblur="saveHeader()">
						...
						<div class="col-md-12">
							<div class="card shadow mb-4">
								<div class="card-header py-2" style="background-color: #2c3e50; color: white;">
									<h6 class="m-0 font-weight-bold"><i class="fas fa-search-plus mr-2"></i> Add Items to Receipt</h6>
								</div>
								<div class="card-body">
									<div class="row">
										<div class="col-md-6">
											<label style="font-weight:bold; color:#e74c3c;"><i class="fas fa-search mr-1"></i> Search Individual Item</label>
									<select class="form-control" name="item_search" id="item_search" multiple="multiple"
										<?php echo $final_disabler; ?> style="max-width:700px!important">
										...
										while ($row1 = $result->fetch_assoc()) { ?>
											<option
												value="<?php echo $row1["item_name"] ?>~<?php echo $row1["item_description"] ?>~<?php echo $row1["cost"] ?>~<?php echo $row1["purchase_tax_cgst"] ?>~<?php echo $row1["purchase_tax_igst"] ?>~<?php echo $row1["item_id"] ?>~<?php echo $row1["hsn"] ?>">
										...
										<div class="col-md-6">
											<label style="font-weight:bold; color:#2980b9;"><i class="fas fa-boxes mr-1"></i> Search Group / Kit</label>
									<select class="form-control" name="group_search" id="group_search"
										multiple="multiple" <?php echo $final_disabler; ?>
										style="max-width:700px!important">
										<?php $sql = "SELECT itemgroup_id, upper(itemgroup_name) itemgroup_name FROM itemgroup ORDER BY upper(itemgroup_name)";
										$result = $conn->query($sql);
										while ($row1 = $result->fetch_assoc()) { ?>
											<option
												value="<?php echo $row1["itemgroup_id"] ?>~<?php echo $row1["itemgroup_name"] ?>">
												<?php echo $row1["itemgroup_name"] ?>
											</option>
										<?php } ?>
									</select>
										</div>
											...
											<tr class="purchase_rows" id="row_<?php echo $slno; ?>"
												...
												<td>
													<div class="tooltip-wrapper"><input class="form-control tooltip-input"
															<?php echo $final_disabler; ?> id="qty_<?php echo $slno; ?>"
												...
												<td><input class="form-control" <?php echo $final_disabler; ?> readonly
														value="<?php echo $row_det['cost']; ?>"
														id="cost_<?php echo $slno; ?>" value="<?php echo $det_cost; ?>"
														onblur="calculate('<?php echo $slno; ?>')"></td>
												...
												<td style="display:none"><input style="text-align:right" <?php echo $final_disabler; ?> class="form-control" readonly
														value="<?php echo $tax_pc_sgst; ?>" id="sgst_<?php echo $slno; ?>">
												</td>
												<td style="display:none"><input style="text-align:right" <?php echo $final_disabler; ?> class="form-control" readonly
														value="<?php echo $tax_pc_igst; ?>" id="igst_<?php echo $slno; ?>">
												</td>
												...
												<td><input class="form-control" <?php echo $final_disabler; ?> readonly
													...
													<input class="form-control" <?php echo $final_disabler; ?> type="hidden"
														id="id_<?php echo $slno; ?>"
														value="<?php echo $det_subtrans_id; ?>">
												</td>
												<td><input class="form-control" <?php echo $final_disabler; ?>
														id="roundoff_<?php echo $slno; ?>"
											...
											<tr class="purchase_rows" id="row_<?php echo $slno; ?>" style="display:none"
												...
												<td>
													<div class="tooltip-wrapper">
														<input class="form-control tooltip-input qty-box"
												...
												<td><input class="form-control amount-box" readonly
														id="cost_<?php echo $slno; ?>"></td>
												...
												<td style="display:<?php echo $visibility; ?>"><input
														class="form-control amount-box" readonly
												...
												<td><input class="form-control amount-box"
														id="discount_<?php echo $slno; ?>"
														onblur="calculate('<?php echo $slno; ?>')"></td>

												<td>
													<input class="form-control amount-box" readonly
												...
												<td><input class="form-control amount-box" readonly
														id="show_total_<?php echo $slno; ?>"></td>
		...
		$(function () {
			...
			$('#group_search').searchableOptionList({
				...
				onChange: function () {
					$('.sol-container').removeClass('sol-active');
					setTimeout(function () {
						$('#group_search').val(null);
						$('#group_search option').prop("selected", false);
						$('.sol-checkbox').prop("checked", false);
						$('.sol-option').removeClass("sol-selected");
					}, 50);
					select_group();
...
/*$(document).ready(function() {
	 let module="<?php echo $module; ?>"; 
				...
				$.post("fetch_items.php", { itemgroup_id: item_id }, function (data, status) {
					if (data && $.trim(data) !== "" && $.trim(data) !== "0") {
						...
						for (let i = 0; i < list.length - 1; i++) {
							...
							// sub[0]=item_id, sub[1]=item_name, sub[2]=tax_pc, sub[3]=tax_pc_sgst, sub[4]=perc
							// Only add if it's NOT the main item itself (to avoid duplicates)
							if (sub[0] != item_id) {
								load_service(sub[0], 0, sub[2], sub[2], sub[1], "");
							}
						}
					} else {
		...
		function select_group() {
			...
			$.post("fetch_items.php", { itemgroup_id: group_id }, function (data, status) {
				if (data && $.trim(data) !== "" && $.trim(data) !== "0") {
					let list = $.trim(data).split("@");
					for (let i = 0; i < list.length - 1; i++) {
						let sub = list[i].split("~");
