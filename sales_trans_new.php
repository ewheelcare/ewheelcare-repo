...
.cust-option:hover { background: #f0f6ff; }
.cust-name { font-weight: 600; font-size: 14px; color: #1e293b; }
...
#create_customer_btn:hover { background: #dbeafe; }
</style>
</head>
    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
				...
				$trans_id=""; $trans_date=""; $details=""; $customer=""; $trans_amount=""; $pending=""; $gst=""; $tally=""; $created_by=""; $created_on=""; $modified_by=""; $modified_on=""; $active_status=""; $VEHICLE_NO=""; $VEHICLE_MODEL=""; $NO_OF_WHEELS=""; $COMPANY_NAME=""; $CUSTOMER_NAME=""; $CUSTOMER_ADDRESS=""; $CUSTOMER_GST=""; $VEHICLE_ODOMETER=""; $VEHICLE=""; $CUSTOMER_MOBILE="";
...
if ($result && $row = $result->fetch_assoc()) {$trans_id=$row["trans_id"];
...
<table class="table">
<tr><td>Date <input type="text" id="datepicker" class="form-control" value="<?php echo !empty($trans_date) ? date('d-m-Y', strtotime($trans_date)) : ''; ?>"></td>
...
<?php if($customer_locked){ ?>
    ...
    <div class="form-control" id="customer_display" style="background:#f8f9fa;">
        ...
        <?php if(!empty($CUSTOMER_NAME) && !empty($COMPANY_NAME)){ echo " (".$CUSTOMER_NAME.")"; } ?>
    </div>
    ...
    <div style="position:relative; max-width:360px;">
        ...
        <div id="customer_dropdown">
            ...
            <div class="cust-option"
                ...
                <div class="cust-sub"><?php echo $sub; ?><?php echo $mobile ? " · ".$mobile : ""; ?></div>
            </div>
            <?php } ?>
            <div id="create_customer_btn" onclick="openCreateCustomerModal()">
                <span style="font-size:18px;line-height:1;">+</span> Create New Customer
            </div>
        </div>
    </div>
    <input type="hidden" id="customer" name="customer" value="<?php echo $customer;?>">
...
<table class="table">
<tr><td>Company Name<input class="form-control" name="company_name" id="company_name" value="<?php echo $COMPANY_NAME;?>" <?php echo $customer_locked ? "readonly" : ""; ?>></td>
<td>Customer Name <input class="form-control" name="customer_name" id="customer_name" value="<?php echo $CUSTOMER_NAME;?>" <?php echo $customer_locked ? "readonly" : ""; ?>></td>
<td>Mobile <input class="form-control" name="customer_mobile" id="customer_mobile" value="<?php echo $CUSTOMER_MOBILE;?>" <?php echo $customer_locked ? "readonly" : ""; ?>></td>
</tr>
...
<?php
$sql="SELECT item_id,item_name,item_description,cost,ifnull(tax_pc,9) tax_pc,ifnull(tax_pc_sgst,9) tax_pc_sgst FROM item where status='A' order by item_id";
$result = $conn->query($sql);
...
$sql_det="SELECT t.subtrans_id,t.item_id,t.cost,price,ifnull(t.discount,'0') discount,t.qty,t.total, t.tax,t.tax_amount,t.tax_sgst,t.total,t.remarks,t.tax_amount_sgst,t.tax_igst,t.tax_amount_igst,i.item_name,t.price,t.roundoff,t.account,t.parent,t.perc FROM sales_trans_det t,item i where i.item_id=t.item_id and t.trans_id='".$trans_id."' and t.trans_id!='' and t.subtrans_id!='' and t.qty>0 and total>0 and t.active_status='A'  and t.ver='".$ver."'
ORDER BY
CASE
...
$readonly="";
$slno=0;
$tax_pc=0; $tax_pc_sgst=0; $tax_pc_igst=0;
$det_tax=0; $det_tax_sgst=0; $det_tax_igst=0;
...
<input type="hidden" id="perc_<?php echo $slno;?>" value="<?php echo $det_perc;?>" >
	...
	<td><div class="tooltip-wrapper"><input class="form-control tooltip-input"  <?php echo $final_disabler;?>  id="qty_<?php echo $slno;?>" onblur="calculate('<?php echo $slno;?>')" value="<?php echo $det_qty;?>">
</div></td>
	...
	<td><input class="form-control" <?php echo $final_disabler;?> readonly value="<?php echo $det_cost;?>" id="cost_<?php echo $slno;?>" onblur="calculate('<?php echo $slno;?>')"></td>
...
<input class="form-control"
       <?php echo $final_disabler;?>
       id="total_<?php echo $slno;?>"
...
<input type="hidden" id="perc_<?php echo $slno;?>" value="" >
	...
	<td><div class="tooltip-wrapper"><input class="form-control tooltip-input"  id="qty_<?php echo $slno;?>" onblur="calculate('<?php echo $slno;?>')">
...
Round Off <input id="roundoff_amount" class="form-control"  value="<?php echo $roundoff_amount;?>" onblur="calculate_final()"><br>Total Invoice Amount :   <input id="final_amount" class="form-control" readonly value="<?php echo $grand_total-($discount_amount)+($roundoff_amount);?>">
</TD>
</tr>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <tr>
                    ...
                    <td><input type="text" id="nc_company_name" class="form-control" placeholder="e.g. ABC Pvt Ltd"></td>
                </tr>
                <tr>
                    <td style="font-weight:600;vertical-align:middle;">Customer Name <span style="color:red;">*</span></td>
                    <td><input type="text" id="nc_customer_name" class="form-control" placeholder="Owner / Contact name"></td>
                </tr>
                <tr>
                    <td style="font-weight:600;vertical-align:middle;">Mobile <span style="color:red;">*</span></td>
                    <td><input type="text" id="nc_mobile" class="form-control" placeholder="Mobile number" maxlength="15"></td>
                </tr>
                <tr>
                ...
                <tr style="display:<?php echo $gst_row_display;?>">
                    ...
                    <td><input type="text" id="nc_gst" class="form-control" placeholder="e.g. 37AAAAA0000A1Z5" maxlength="15"></td>
                </tr>
                </table>
...
function saveNewCustomer() {
    ...
    var errEl        = document.getElementById("nc_error_msg");

    errEl.style.display = "none";
    errEl.innerText = "";

    if (!customerName) { errEl.innerText = "Customer Name is required."; errEl.style.display = "block"; return; }
    if (!mobile)       { errEl.innerText = "Mobile number is required.";  errEl.style.display = "block"; return; }

    var saveBtn = document.getElementById("nc_save_btn");
    saveBtn.disabled  = true;
    saveBtn.innerText = "Saving...";

    $.post("add_customer.php", {
        company_name: companyName, customer_name: customerName,
        customer_mobile: mobile, customer_address: address, customer_gst: gstNo
    }, function(data) {
        ...
        if (data.indexOf("Error") !== -1) {
            errEl.innerText = data; errEl.style.display = "block"; return;
        }

        var parts        = data.split("~");
        var status       = parts[0]; // SUCCESS or EXISTS
        var customerId   = parts[1] || "";
        var retCompany   = parts[2] || companyName;
        var retName      = parts[3] || customerName;
        var retMobile    = parts[4] || mobile;

        if (status === "EXISTS") {
            errEl.innerText = "Customer already exists — selecting them now.";
            errEl.style.display = "block";
        }

        // Fill all fields
        document.getElementById("customer").value        = customerId;
        document.getElementById("company_name").value    = retCompany;
        document.getElementById("customer_name").value   = retName;
        document.getElementById("customer_mobile").value = retMobile;
        document.getElementById("customer_search_input").value =
            retCompany ? retCompany + (retName ? " (" + retName + ")" : "") : retName;

        // Add to dropdown list immediately
        var newOpt = document.createElement("div");
        newOpt.className = "cust-option";
        newOpt.setAttribute("data-val", customerId + "~" + retCompany + "~" + retName + "~" + retMobile);
        newOpt.setAttribute("data-search", (retCompany + " " + retName + " " + retMobile).toLowerCase());
        newOpt.setAttribute("onclick", "pickCustomer(this)");
        newOpt.innerHTML = '<div class="cust-name">' + (retCompany || retName) + '</div>'
            + '<div class="cust-sub">' + retName + (retMobile ? ' · ' + retMobile : '') + '</div>';
        var createBtn = document.getElementById("create_customer_btn");
        createBtn.parentNode.insertBefore(newOpt, createBtn);

        get_gst();
        get_address();

        setTimeout(function() { createCustomerModal.hide(); }, status === "EXISTS" ? 1200 : 400);
    });
}
<?php } ?>
/* ── END CHANGE 3 JS ── */
...
function add_service_det(service_id){
 ...
 $.post("add_sales_det.php",
  ...
  function(data, status){
 if($.trim(data).indexOf("Error")==-1 && $.trim(data).indexOf("Alert")==-1 && $.trim(data).indexOf("Available Stock")==-1){
  ...
  function(data, status){
   ...
   alert("Transaction unlocked with ID: "+$("#trans_id").val());
	...
	function calculate_final(){
		...
		let roundoff = parseFloat(document.getElementById("roundoff_amount").value || 0);
		let final_amount = trans_amount - discount + roundoff;
