    ...
    <div class="receipt-container" id="content-to-pdf">
        ...
        <table class="items-table">
            ...
            <tbody>
                ...
                $roundoff = isset($row_m["roundoff"]) ? floatval($row_m["roundoff"]) : 0;
            ...
            <table class="items-table" style="margin-top: 5px; font-size: 10px;">
                ...
                <tbody>
                    ...
                    while ($row = $res_d->fetch_assoc()) {
                        ...
                        $hsn_summary[$key]["price"] += $row["price"];
                        $hsn_summary[$key]["tax_amount"] += $row["tax_amount"];
                        $hsn_summary[$key]["tax_amount_sgst"] += $row["tax_amount_sgst"];
                        $hsn_summary[$key]["tax_amount_igst"] += $row["tax_amount_igst"];
                    }

                    foreach ($hsn_summary as $summary_row) {
                        $tax_amount_sgst += $summary_row["tax_amount_sgst"];
                        $tax_amount_cgst += $summary_row["tax_amount"];
                        $tax_amount_igst += $summary_row["tax_amount_igst"];
                        $total_taxable += $summary_row["price"];
                        $row_total_tax = $summary_row["tax_amount"] + $summary_row["tax_amount_sgst"] + $summary_row["tax_amount_igst"];
