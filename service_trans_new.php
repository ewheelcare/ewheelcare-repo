					...
					<div class="row">
						...
						<div class="col-md-12">
							...
							<table class="table">
								...
								<tr>
									<td>Vehicle Make <input class="form-control" name="vehicle_make" id="vehicle_make" list="make_list" value="<?php echo $VEHICLE_MAKE; ?>" autocomplete="off" <?php echo $final_disabler; ?>>
									...
									<td>Vehicle Model <input class="form-control" name="vehicle_model" id="vehicle_model" list="model_list" value="<?php echo $VEHICLE_MODEL; ?>" autocomplete="off" <?php echo $final_disabler; ?>>
									...
									<td>No of Wheels <input class="form-control" name="no_of_wheels" id="no_of_wheels" value="<?php echo $NO_OF_WHEELS; ?>" onblur="setwheels()" <?php echo $final_disabler; ?>></td>
									<td>Odometer <input class="form-control" name="vehicle_odometer" id="vehicle_odometer" value="<?php echo $VEHICLE_ODOMETER; ?>" <?php echo $final_disabler; ?>></td>
								</tr>
						...
						<div class="col-md-12">
							...
							<table class="table">
								<tr>
									...
									<td>Company Name<input class="form-control" name="company_name" id="company_name" value="<?php echo $COMPANY_NAME; ?>" LIST="cust_list" <?php echo $final_disabler; ?>></td>
									<td>Customer Name <input class="form-control" name="customer_name" id="customer_name" value="<?php echo $CUSTOMER_NAME; ?>" <?php echo $final_disabler; ?>></td>
									<td>Customer Mobile
										<input class="form-control" name="customer_mobile" id="customer_mobile"
											...
											oninput="this.value=this.value.replace(/\D/g,'')" placeholder="" <?php echo $final_disabler; ?>>
									</td>
								</tr>
								<tr>
									<td colspan="1" style="display:<?php echo $visibility; ?>">
										GST <input class="form-control" name="customer_gst" id="customer_gst"
											...
											onfocus="get_gst()" autocomplete="off" onblur="set_gst()" <?php echo $final_disabler; ?>>
										<datalist id="gst_list"></datalist>
									</td>
									<td colspan="2">Address <input class="form-control" name="customer_address" id="customer_address" value="<?php echo $CUSTOMER_ADDRESS; ?>" list="address_list" onfocus="get_address()" autocomplete="off" <?php echo $final_disabler; ?>>
										<datalist id="address_list"></datalist>
										<input class="form-control" name="customer" id="customer" value="<?php echo $customer; ?>" type="hidden">
									</td>
									<td>
										<td colspan="1">Mechanic <input class="form-control" name="mech" id="mech" value="<?php echo $mech; ?>" list="mech_list" <?php echo $final_disabler; ?>>
...
<table class="table" id="payment_table">
	...
	<script>
		...
		$(function() {
			...
			// Payment check for Print buttons
			let already_paid = parseFloat($("#already_paid").val()) || 0;
			if (already_paid <= 0) {
				$(".btn-warning, #invoice").css("opacity", "0.6").attr("title", "Add payment to enable printing");
			}
		});
...
let taxable   = (parseFloat(price) * parseFloat(qty)) - parseFloat(discount || 0);
...
$.post("add_service_det.php", {
    ...
    $("#del_" + service).prop('disabled', false);
		...
		function calculate(service) {
			...
			let gross_cost = price * qty;
			let taxable_value = gross_cost - discount;
			if (taxable_value < 0) taxable_value = 0;

			let tax_gst  = taxable_value * gst  / 100;
			let tax_sgst = taxable_value * sgst / 100;
			let total    = taxable_value + tax_gst + tax_sgst;

			$("#cost_"     + service).val(gross_cost.toFixed(2));
...
function load_special_cost(service) {
    ...
    $(".row_service:visible").each(function () {
        ...
        if (qty > 0) {
            ...
            let p   = parseFloat($("#price_"    + service).val()) || 0;
            let q   = parseFloat($("#qty_"      + service).val()) || 0;
            let d   = parseFloat($("#discount_" + service).val()) || 0;
            let tax_val = (p * q) - d;
...
function setwheels() {
   ...
   // add_service_det("1000002");
		...
		function get_invoice(param) {
			...
			if (already_paid <= 0) {
				alert("❌ Payment Missing!\n\nPlease add at least one payment before printing the invoice. This ensures payments are not missed.");
				return;
			}
		...
		function filter_account1() {
			let pay_type_val = document.getElementById("pay_type1").value;
			if (!pay_type_val) {
				$('#account1 option').prop('disabled', false);
				return;
			}
			let pay_type = pay_type_val.split("~")[0];
			
			// FIRST: Re-enable all options so we can filter fresh
			$('#account1 option').prop('disabled', false);

			// SECOND: Disable options that don't EXACTLY match the selected pay type ID
			...
			// THIRD: If the currently selected account is now disabled, clear it
			if ($('#account1 option:selected').is(':disabled')) {
				$('#account1').val("");
			}

			// Fourth: Special default for Cash
			if (pay_type_val.includes("CASH")) {
