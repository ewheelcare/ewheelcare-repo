...
<body id="content-to-pdf">
    <div class="no-print" style="background: #e52d27; padding: 10px; display: flex; justify-content: space-between; align-items: center; color: white; font-family: sans-serif;">
        ...
        <button type="button" onclick="window.location.href='receipt.php'" 
...
$sql="SELECT trans_id,trans_date,details,trans_amount,pending,m.vendor,'Y' gst,v.company_name,v.company_address,v.owner_name,v.owner_mobile,v.owner_aadhar FROM receipt_trans m,vendor v where v.vendor_id=m.vendor and m.active_status='A' and m.trans_id='".$trans_id."' order by 1";
echo $sql;
...
<tr><td><table style="width:100%; table-layout:fixed;border:1px solid #4a4a4a">
<tr><td style="width:50%;border:1px solid #4a4a4a">Vendor :<?php echo $company_name;?><br>
...
<?php  $sql="SELECT trans_id,subtrans_id, i.item_id,i.item_name,i.item_description,d.cost,d.qty,d.tax,d.tax_amount,d.total,(d.tax_amount+d.total) full_amount FROM receipt_trans_det d,item i where i.item_id=d.item_id and d.trans_id='".$trans_id."' and d.active_status='A'";
				//echo $sql;
				$result = $conn->query($sql);
$tax_amount=0;
$total_amount=0;
