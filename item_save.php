...
if ($hsn === '') {
    echo 'HSN is required.';
    exit;
}

if (!isValidNum($cost)) {
    echo 'Cost must be a valid non-negative number.';
    exit;
}
if (!isValidPerc($tax_pc)) {
    echo 'Tax CGST must be a number between 0 and 100.';
    exit;
}
if (!isValidPerc($tax_pc_sgst)) {
    echo 'Tax SGST must be a number between 0 and 100.';
    exit;
}
if (!isValidPerc($purchase_tax_cgst)) {
    echo 'Purchase Tax CGST must be a number 0–100.';
    exit;
}
if (!isValidPerc($purchase_tax_igst)) {
    echo 'Purchase Tax IGST must be a number 0–100.';
    exit;
}
...
if ($has_subitems && count($sub_items) > 0) {
    ...
    foreach ($sub_items as $idx => $si) {
        ...
        if (trim($si['item_name'] ?? '') === '') {
            echo "Sub Item $n: Name is required.";
            exit;
        }
        if ($si_hsn === '') {
            echo "Sub Item $n: HSN is required.";
            exit;
        }
        if (!isValidNum($si_cost)) {
            echo "Sub Item $n: Cost must be a valid non-negative number.";
            exit;
        }
        if (!isValidPerc($si_perc)) {
            echo "Sub Item $n: Percentage must be a number between 0 and 100.";
            exit;
        }
    ...
    if ($total_perc > 100) {
        echo 'Sub item percentages cannot exceed 100%.';
        exit;
    }
}
