...
<button class="btn btn-sm btn-primary" onclick="print_table()" style="float:right;margin:3px;">Excel</button>

<div style="overflow-x:auto;">
<table class="table table-striped table-bordered" id="report_table" style="font-size:75%;font-weight:bold;text-transform:uppercase">

<thead>
<tr style="background:#f0f0f0 font-weight:bold; font-size:11px;" >
<th style="padding:3px;">Shop</th>
<?php foreach($serv_array as $sid){ ?>
<th style="padding:3px;"><?= $serv_names[$sid] ?></th>
<?php } ?>
...
<tr>
<td>Total</td>
<?php foreach($serv_total_array as $v){ ?>
<td><?= $v ?></td>
<?php } ?>
<td><?= $overall ?></td>
</tr>
