...
<body id="page-top">
	...
	$trans_id=isset($_GET["trans_id"])?$_GET["trans_id"]:"";
					$sql="SELECT trans_id, trans_date, details, customer, trans_amount, pending, gst, tally, created_by, created_on, modified_by, modified_on, active_status, COMPANY_NAME, CUSTOMER_NAME, CUSTOMER_ADDRESS, CUSTOMER_GST, customer_mobile FROM sales_trans where trans_id='".$trans_id."'";
//echo $sql;
...
<td>Invoice No  <span id="year_part"></span><input class="form-control" readonly name="trans_id" id="trans_id" value="<?php echo $trans_id;?>"></td>
...
<?php  $sql="SELECT item_id,item_name,item_description,cost,ifnull(tax_pc,9) tax_pc,ifnull(tax_pc_sgst,9) tax_pc_sgst FROM item order by item_id";
$result = $conn->query($sql);
while ($row1 = $result->fetch_assoc()) {?>
...
<td><input class="form-control" <?php echo $final_disabler;?>   id="total_<?php echo $slno;?>" value="">
...
<TD><input id="discount_amount" class="form-control" readonly value="<?php echo $discount_amount;?>"></TD>
...
function calculate(service){
 ...
 tax_perc_igst=0;
	...
	document.getElementById(cost_selector).value=parseFloat(document.getElementById(price_selector).value)/parseFloat((document.getElementById(qty_selector).value));
